...
bool Scene::Update(float dt)
{
	...
	//el player vuelve al primer checkpoint 
	...
	//el player vuelve al segundo checkpoint
	...
	//el player respawnea al principio
	if (app->input->GetKey(SDL_SCANCODE_F3) == KEY_DOWN && player->isAlive && player->fell == false){
		...
		{
			app->entityManager->DestroyEntity(app->entityManager->entities[i]);

			app->physics->ChupaBody(app->physics->GetWorld(),app->entityManager->entities[i]->pbody->body);
		}
		checkpoint = 0;
		Awake(savedConfig);
	}
